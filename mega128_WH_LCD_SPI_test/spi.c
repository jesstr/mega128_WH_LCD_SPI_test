...
#include "spi.h"
  ...
  {
	SPI_DDR |= _BV(CS1_PIN) | _BV(SCK_PIN) | _BV(MOSI_PIN);	
  ...
  {
	SPI_DDR = (1<<MISO_PIN);
	//SPCR = (1<<SPE)|(1<<CPHA); 	/* Enable SPI, Slave, Mode 1 */
	SPCR = (1<<SPE)|(1<<SPIE);; 	/* Enable SPI, Slave, Mode 0, Interrupt enabled */
